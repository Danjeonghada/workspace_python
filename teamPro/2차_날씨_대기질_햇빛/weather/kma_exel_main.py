...
# ✅ 사용할 엑셀 파일명
EXCEL_FILE = "../air_quality/xy_code_daejeon.xlsx"
...
# ✅ 엑셀에서 동 이름 기반 좌표 찾기
def get_coords_from_excel(dong_name):
    df = pd.read_excel(EXCEL_FILE)

    row = df[df['3단계'] == dong_name]  # ✅ 이 줄만 사용
    if row.empty:
        print(f"[오류] '{dong_name}'의 좌표 정보를 엑셀에서 찾을 수 없습니다.")
        return None
    return int(row.iloc[0]['격자 X']), int(row.iloc[0]['격자 Y'])

# ✅ 날씨 API 요청
def get_weather_by_dong(dong_name):
    coords = get_coords_from_excel(dong_name)
    if coords is None:
        return
